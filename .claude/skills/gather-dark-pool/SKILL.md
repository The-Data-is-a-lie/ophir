...
- **Extend:** add FINRA ATS *weekly* per-ATS detail (which dark pool; 2–4-week lag; free FINRA API)
  as a sibling fetcher; or wire `dark_pool_signal` into `ResearchBrief` as a `flow` dimension surfaced
  in the research / bull-bear debate / manager prompts (advisory) — the planned next step.
