...
Builds the Typer application exposed as the ``ophir`` console script (entry
point ``ophir.cli:app``). It mounts the :mod:`ophir.register` and
:mod:`ophir.trading` sub-applications and the agent ``intraday`` / ``report``
sub-apps, and adds the model commands (``train``/``finetune``/``evaluate``/
``curate``/``sweep``/``dashboard``/``migrate-sqlite``) alongside the agent
commands (``predict``/``decide``/``research``/``manage``/``trade``/``backtest``).
"""
...
# Upstream's deterministic ophir.trading core (propose/gate/record/close/
# performance), namespaced under `trading` so it does not collide with the
# agent-driven `trade` command below.
app.add_typer(trading_cli.app, name="trading")
...
@app.command(name="fred-key")
def fred_key(key: str) -> None:
    """Store a free FRED API key for the macro credit / conditions / curve series.

    Persisted under the ophir data dir and read by the macro signal
    (:mod:`ophir.agent.macro`); without it the macro signal uses its VIX-only
    path. Relocated from ``ophir register fred-key`` so the upstream ``register``
    ...
    """
    from ophir.agent.fred import set_fred_key

    set_fred_key(key)
    typer.echo("FRED API key stored.")
